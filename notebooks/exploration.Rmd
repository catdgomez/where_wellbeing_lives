...
barplot(table(social_support_or_lack$TIME_PERIOD), main = "frequencyasdf  of years in the data")
```
...
phs_v1_bridge <- phs[order(phs$`Reference area`),] 
phs_v1 <- phs_v1_bridge[1:100, ]

el_v1_bridge <- el[order(el$`Reference area`),]
el_v1 <- el_v1_bridge[1:250, ]

safety_v1_bridge <- safety[order(safety$`Reference area`),]
safety_v1 <- safety_v1_bridge[1:250, ]

social_support_or_lack_v1_bridge <- social_support_or_lack[order(social_support_or_lack$`Reference area`),]
social_support_or_lack_v1 <- social_support_or_lack_v1_bridge[1:250, ]

lonely_v1_bridge <- lonely[order(lonely$`Reference area`),]
lonely_v1 <- lonely_v1_bridge[1:73,]

phs_v1
el_v1
safety_v1
social_support_or_lack_v1
lonely_v1
...
Merge all three datasets together using purr package, reduce

```{r}

list(phs_v3, el_v3, safety_v3, social_support_or_lack_v3) %>% 
  reduce(left_join, by = c("Reference_area", "TIME_PERIOD", "REF_AREA"))
...
Merge the ordered data sets on the country (REF_AREA) and year (TIME_PERIOD) columns.

```{r}

# merge two datasets by country and year
phs_el_v1 <- merge(phs_v3, el_v3, by = c("Reference_area", "TIME_PERIOD", "REF_AREA"))
...
Order country column for safety data
