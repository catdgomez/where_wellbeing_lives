...
library(reshape)
library(tidyr)
# for scatter plot with correlation to side
...
the_values <- wellbeing_df_dropped[, 5:17]#[, c("life_satisfaction", "feeling_lonely", "social_support", "intentional_homicides", "satisfaction_with_personal_relationships")] #colnames(wellbeing_df_dropped)[5:17]
the_values 
data <- cor(the_values)
...
ggplot(data_melted, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  labs(title = "Heatmap Of relationship between four variables") +
  scale_fill_gradient2(low = "yellow", high = "red",
                       limit = c(-1,1), name="value") +
  geom_text(aes(Var2, Var1, label = round(value, digits = 2)), size = 3) +
...
wellbeing_lm <- lm(life_satisfaction ~ feeling_lonely + social_support + intentional_homicides + tertiary_education + health_status_F, wellbeing_df_dropped)
# summary(wellbeing_lm)

# confint(wellbeing_lm)
...
new_data <- data.frame(wellbeing_df_dropped$life_satisfaction)
...
mean.pred.intervals(wellbeing_df_dropped$life_satisfaction, wellbeing_df_dropped$health_status_F, new_data)
...
scatter_lonely_homicides <- ggplot(
      wellbeing_df_dropped, aes(x = feeling_lonely, y = intentional_homicides)) + 
      geom_point() +
      geom_smooth(method = "lm", se = TRUE) +
      labs(title = "Rel between variables")
...
res <- cor.test(wellbeing_df_dropped$intentional_homicides, wellbeing_df_dropped$feeling_lonely, 
                     method = "pearson")
# res
# # Extract the p.value
 res$p.value
...
Pvalue <- paste(strwrap("Unemployment rates in the US have 
  varied a lot over the years", 40), collapse = "\n")

ggscatter(wellbeing_df_dropped, x = "intentional_homicides", y = "feeling_lonely", 
          add = "reg.line", conf.int = TRUE, 
          cor.ctes = TRUE, cor.method = "pearson") +
          annotate(geom = "text", 
                   x = wellbeing_df_dropped$intentional_homicides,
                   y=wellbeing_df_dropped$feeling_lonely[1], 
                   label= Pvalue) #+
          # annotate("text", x=wellbeing_df_dropped$intentional_homicides, y=wellbeing_df_dropped$feeling_lonely[1], label= res$p.value) + 
          # annotate("text", x=wellbeing_df_dropped$intentional_homicides, y=wellbeing_df_dropped$feeling_lonely[1], label= "Correlation Coefficient") +
          # annotate("text", x = wellbeing_df_dropped$intentional_homicides, y=wellbeing_df_dropped$feeling_lonely[1], label = res$estimate)
          # # #,
          #xlab = "Loneliness vs Intentional Homicides", ylab = "Weight (1000 lbs)")
```

```{r}
Pvalue <- paste(strwrap("P-value", 40), collapse = "\n")

ggscatter(wellbeing_df_dropped, 
          x = "intentional_homicides", 
          y = "feeling_lonely") +
  # geom_text() +
  annotate(
    "text", label = Pvalue,
    x = -Inf, y = -Inf, hjust = 0, vjust = 0, size = 8, colour = "red"
  )
```
...
phs_v3 |> 
      ggplot(aes(x = OBS_VALUE_PHS)) +
      geom_histogram(color = "white", fill = "salmon") + 
      labs(title = "title")
```
```{r}
write.csv(phs_v3, file = "../data/phs_grouped.csv")

write.csv(el_v3, file = "../data/el_grouped.csv")

write.csv(safety_v3, file = "../data/safety_grouped.csv")

write.csv(social_support_or_lack_v3, file = "../data/social_grouped.csv")

write.csv(satisfaction_life_rels_v3, file = "../data/sat_life_grouped.csv")
...
p<-ggplot(phs_v3, aes(x=OBS_VALUE_PHS))+
  geom_histogram(color="white", fill="salmon")+
...
wellbeing_df_dropped %>% 
  correlate() %>% 
  focus(life_satisfaction)

```
```{r}

result <- t.test(wellbeing_df_dropped$life_satisfaction)
...
hmm <- cor.test(wellbeing_df_dropped[4:17] %>% 
                      pull(), 
                    wellbeing_df_dropped[4:17] %>% 
                      pull(), 
                    method = "pearson")

wellbeing_df_dropped %>% view()
...
ct <- cor.test(
  wellbeing_df_dropped$life_satisfaction, 
  wellbeing_df_dropped$intentional_homicides,
  method = "pearson"
  )
# ct
ct
round(ct$conf.int[1], 3)
...
est <- map_dbl(
  ...
  list(wellbeing_df_dropped$feeling_lonely, wellbeing_df_dropped$social_support, wellbeing_df_dropped$intentional_homicides, wellbeing_df_dropped$health_status_F, wellbeing_df_dropped$health_status_B, wellbeing_df_dropped$health_status_G, wellbeing_df_dropped$upper_secondary_education, wellbeing_df_dropped$tertiary_education, wellbeing_df_dropped$below_upper_secondary_education, wellbeing_df_dropped$motor_vehicle_theft, wellbeing_df_dropped$mortality_from_transport_accidents, wellbeing_df_dropped$lack_of_social_support, wellbeing_df_dropped$satisfaction_w_relationships),
  
  \(x) round(cor.test(wellbeing_df_dropped$life_satisfaction, x, method = "pearson")$estimate, 4)
...
lower <- map_dbl(
  ...
  \(x) round(cor.test(wellbeing_df_dropped$life_satisfaction, x, method = "pearson")$conf.int[1], 4)
...
upper <- map_dbl(
  ...
  list(wellbeing_df_dropped$feeling_lonely, wellbeing_df_dropped$social_support, wellbeing_df_dropped$intentional_homicides, wellbeing_df_dropped$health_status_F, wellbeing_df_dropped$health_status_B, wellbeing_df_dropped$health_status_G, wellbeing_df_dropped$upper_secondary_education, wellbeing_df_dropped$tertiary_education, wellbeing_df_dropped$below_upper_secondary_education, wellbeing_df_dropped$motor_vehicle_theft, aQ1wellbeing_df_dropped$mortality_from_transport_accidents, wellbeing_df_dropped$lack_of_social_support, wellbeing_df_dropped$satisfaction_w_relationships),
  
  \(x) round(cor.test(wellbeing_df_dropped$life_satisfaction, x, method = "pearson")$conf.int[2], 4)
